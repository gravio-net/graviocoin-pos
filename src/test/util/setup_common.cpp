...
BasicTestingSetup::BasicTestingSetup(const std::string& chainName, bool fGraviocoinModeIn)
    : m_path_root{fs::temp_directory_path() / "test_common_" PACKAGE_NAME / std::to_string(g_insecure_rand_ctx_temp_path.rand32())}
{
    fGraviocoinMode = fGraviocoinModeIn;

    fs::create_directories(m_path_root);
    gArgs.ForceSetArg("-datadir", m_path_root.string());
    ClearDatadirCache();
    SelectParams(chainName);
    ResetParams(chainName, fGraviocoinMode);
    SeedInsecureRand();
...
TestingSetup::TestingSetup(const std::string& chainName, bool fGraviocoinModeIn) : BasicTestingSetup(chainName, fGraviocoinModeIn)
{
