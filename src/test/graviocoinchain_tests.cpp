...
BOOST_FIXTURE_TEST_SUITE(graviocoinchain_tests, GraviocoinBasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(graviocoinchain_test)
{
    SeedInsecureRand();
    FillableSigningProvider keystore;
    ...
    CBlock blk;
    blk.nVersion = GIO_BLOCK_VERSION;
    blk.nTime = 1487406900;
    ...
    txnSpend.nVersion = GIO_BLOCK_VERSION;
}
...
BOOST_AUTO_TEST_CASE(mixed_input_types)
{
    CMutableTransaction txn;
    txn.nVersion = GIO_TXN_VERSION;
    BOOST_CHECK(txn.IsGraviocoinVersion());
    ...
    txnPrev.nVersion = GIO_TXN_VERSION;
    BOOST_CHECK(txnPrev.IsGraviocoinVersion());
