...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    bool fIsP2SH = creator.IsGraviocoinVersion()
    ...
    if (creator.IsGraviocoinVersion() && !sigdata.witness) {
        sigdata.scriptWitness.stack = result;
    } else  {
...
public:
    ...
    bool is_graviocoin_tx = false;
    bool IsGraviocoinVersion() const override { return is_graviocoin_tx; }
};
...
SignatureData DataFromTransaction(const CMutableTransaction& tx, unsigned int nIn, const std::vector<uint8_t> &amount, const CScript &scriptPubKey)
{
    ...
    extractor_checker.is_graviocoin_tx = tx.IsGraviocoinVersion();
...
class DummySignatureCheckerGraviocoin : public DummySignatureChecker
{
// IsGraviocoinVersion() must return true to skip stack evaluation
public:
    DummySignatureCheckerGraviocoin() : DummySignatureChecker() {}
    bool IsGraviocoinVersion() const override { return true; }
};
const DummySignatureCheckerGraviocoin DUMMY_CHECKER_GIO;

class DummySignatureCreatorGraviocoin : public DummySignatureCreator {
public:
    DummySignatureCreatorGraviocoin() : DummySignatureCreator(33, 32) {}
    const BaseSignatureChecker& Checker() const override { return DUMMY_CHECKER_GIO; }
    bool IsGraviocoinVersion() const override { return true; }
};
...
const BaseSignatureCreator& DUMMY_SIGNATURE_CREATOR_GIO = DummySignatureCreatorGraviocoin();
