...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    if (checker.IsGraviocoinVersion())
    {
        assert(witness);
        if (scriptSig.size() != 0) {
    ...
    bool fIsP2SH = checker.IsGraviocoinVersion() ? scriptPubKey.IsPayToScriptHashAny(checker.IsCoinStake()) : scriptPubKey.IsPayToScriptHash();
