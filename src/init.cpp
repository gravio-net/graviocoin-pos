...
{
    // Create a message-only window to intercept WM_CLOSE events from graviocoind
 ...
 */
static const char* BITCOIN_PID_FILENAME = "graviocoin.pid";
...
{
#ifdef WIN32
    // Only graviocoind will create a hidden window to receive messages
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-rpccorsdomain=<domain>", "Allow JSON-RPC connections from specified domain (e.g. http://localhost:4200 or \"*\"). This needs to be set if you are using the Graviocoin GUI in a browser.", ArgsManager::ALLOW_ANY, OptionsCategory::RPC);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/graviocoin/graviocoin-core>";
    const std::string URL_WEBSITE = "<https://graviocoin.io/>";
...
{
    fGraviocoinMode = !gArgs.GetBoolArg("-btcmode", false); // qa tests
    if (!fGraviocoinMode) {
        WITNESS_SCALE_FACTOR = WITNESS_SCALE_FACTOR_BTC;
        if (gArgs.GetChainName() == CBaseChainParams::REGTEST) {
            ResetParams(CBaseChainParams::REGTEST, fGraviocoinMode);
        }
    }
...
bool AppInitMain(NodeContext& node)
{
    ...
    if (fGraviocoinMode && gArgs.GetBoolArg("-smsg", true)) { // SMSG breaks functional tests with services flag, see version msg
#ifdef ENABLE_WALLET
...
#ifdef ENABLE_WALLET
    if (fGraviocoinMode && GetWallets().size() > 0) {
