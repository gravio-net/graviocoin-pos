...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    fGraviocoinMode = !gArgs.GetBoolArg("-btcmode", false);
    fCreateBlank = gArgs.GetBoolArg("-create", false);

    if (argc < 2 || HelpRequested(gArgs)) {
        // First part of help message is specific to this utility
        std::string strUsage = PACKAGE_NAME " graviocoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  graviocoin-tx [options] <hex-tx> [commands]  Update hex-encoded transaction\n" +
            "or:     graviocoin-tx [options] -create [commands]   Create hex-encoded transaction\n" +
            "\n";
...
{
    int64_t newVersion;
    if (!ParseInt64(cmdVal, &newVersion) || newVersion < 1 || newVersion > CTransaction::MAX_STANDARD_GIO_VERSION)
        throw std::runtime_error("Invalid TX version requested: '" + cmdVal + "'");

    if (!tx.IsGraviocoinVersion() && IsGraviocoinTxVersion(newVersion)) {
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (tx.IsGraviocoinVersion() && amount == 0) {
...
{
    if (!tx.IsGraviocoinVersion())
        throw std::runtime_error("tx not graviocoin version.");
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        tx.nVersion = CTransaction::CURRENT_GIO_VERSION;
        int startArg;
