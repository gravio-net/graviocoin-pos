...
static const uint8_t GIO_BLOCK_VERSION = 0xA0;
static const uint8_t GIO_TXN_VERSION = 0xA0;
static const uint8_t MAX_GIO_TXN_VERSION = 0xBF;
...
inline bool IsGraviocoinTxVersion(int nVersion)
{
    return (nVersion & 0xFF) >= GIO_TXN_VERSION;
}
...
inline void SerializeTransaction(const TxType& tx, Stream& s) {
    ...
    if (IsGraviocoinTxVersion(tx.nVersion)) {
...
public:
    ...
    static const int32_t CURRENT_GIO_VERSION=0xA0;
    ...
    static const int32_t MAX_STANDARD_GIO_VERSION=0xA0;
...
public:
    ...
    bool IsGraviocoinVersion() const {
        return IsGraviocoinTxVersion(nVersion);
    }
    ...
    {
        return IsGraviocoinTxVersion(nVersion) ? vpout.size() : vout.size();
    }
