    ...
    for (const auto& arg_map : m_available_args) {
        switch(arg_map.first) {
            ...
            case OptionsCategory::SMSG:
                ...
                break;
            case OptionsCategory::GIO_WALLET:
                usage += HelpMessageGroup("Graviocoin wallet Commands:");
                break;
            case OptionsCategory::GIO_STAKING:
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Graviocoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Graviocoin
    // Mac: ~/Library/Application Support/Graviocoin
    // Unix: ~/.graviocoin
#ifdef WIN32
    // Windows
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    const int GIO_START_YEAR = 2017;

    std::string sRange = strprintf(" %i-%i ", GIO_START_YEAR, COPYRIGHT_YEAR);
