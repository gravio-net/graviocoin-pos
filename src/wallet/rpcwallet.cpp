...
void EnsureWalletIsUnlocked(const CWallet* pwallet)
{
    ...
    if (IsGraviocoinWallet(pwallet)
        && GetGraviocoinWallet(pwallet)->fUnlockForStakingOnly)
...
static UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getnewaddress",
                "\nReturns a new Graviocoin address for receiving payments.\n"
                ...
                RPCResult{
            "\"address\"    (string) The new graviocoin address\n"
                },
                RPCExamples{
    ...
    size_t type_ofs = fGraviocoinMode ? 4 : 1;
    ...
    if (IsGraviocoinWallet(pwallet)) {
        ...
        CPubKey newKey;
        CHDWallet *phdw = GetGraviocoinWallet(pwallet);
        {
            LOCK(cs_main);
            {
                LOCK(phdw->cs_wallet);
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getrawchangeaddress",
                "\nReturns a new Graviocoin address, for receiving change.\n"
...
static UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "The graviocoin address to be associated with a label."},
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Graviocoin address");
    }
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"sendtoaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The graviocoin address to send to."},
            ...
            "                             The recipient will receive less graviocoin than you enter in the amount field."},
...
static UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The graviocoin address\n"
        ...
        return NullUniValue;
            ...
            RPCHelpMan{"signmessage",
                "\nSign a message with the private key of an address" +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The graviocoin address to use for the private key."},
...
static UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getreceivedbyaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The graviocoin address for transactions."},
    ...
    for (const std::pair<const uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        ...
        if ((!pwallet->IsGraviocoinWallet() && wtx.IsCoinBase()) || !locked_chain->checkFinalTx(*wtx.tx)) {
            continue;
        }

        if (pwallet->IsGraviocoinWallet()) {
...
static UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{"sendmany",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, RPCArg::Optional::NO, "A json object with addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The graviocoin address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
            ...
            "                           Those recipients will receive less graviocoin than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Graviocoin address: ") + name_);
        }
...
static UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"addmultisigaddress",
                ...
                "Each key is a Graviocoin address or hex-encoded public key.\n"
                "This functionality is only intended for use with non-watchonly addresses.\n"
                "See `importaddress` for watchonly p2sh address support.\n"
                "If 'label' is specified, assign address to that label.\n",
                {
                    {"nrequired", RPCArg::Type::NUM, RPCArg::Optional::NO, "The number of required signatures out of the n keys or addresses."},
                    {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of graviocoin addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "graviocoin address or hex-encoded public key"},
                        },
                        },
    ...
    OutputType output_type = pwallet->m_default_address_type;
    size_t type_ofs = fGraviocoinMode ? 5 : 3;
    if (!request.params[type_ofs].isNull()) {
        if (!ParseOutputType(request.params[type_ofs].get_str(), output_type)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("Unknown address type '%s'", request.params[type_ofs].get_str()));
        }
    ...
    bool fbech32 = fGraviocoinMode && request.params.size() > 3 ? request.params[3].get_bool() : false;
    bool f256Hash = fGraviocoinMode && request.params.size() > 4 ? request.params[4].get_bool() : false;
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth) {
        for (const COutputEntry& r : listReceived)
        {
            ...
            if (pwallet->IsGraviocoinWallet()
                ...
                if (GetGraviocoinWallet(pwallet)->GetStealthLinked(idK, sx)) {
            ...
            if (wtx.IsCoinBase()) {
                ...
                if (wtx.IsImmatureCoinBase()) {
                    ...
                    entry.pushKV("category", (fGraviocoinMode ? "coinbase" : "generate"));
                }
            } else {
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            "    \"address\":\"address\",    (string) The graviocoin address of the transaction.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            "      {\n"
            "        \"involvesWatchonly\": xxx,         (bool) Only returns true if imported addresses were involved in transaction.\n"
            "        \"address\" : \"address\",          (string) The graviocoin address involved in the transaction\n"
            "        \"category\" :                      (string) The transaction category.\n"
            "                     \"send\"                  Transactions sent.\n"
            "                     \"receive\"               Non-coinbase transactions received.\n"
            "                     \"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
...
static UniValue abandontransaction(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->mapWallet.count(hash)) {
        if (!IsGraviocoinWallet(pwallet) || !GetGraviocoinWallet(pwallet)->HaveTransaction(hash)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid or non-wallet transaction id");
        }
    }
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"walletpassphrase",
                ...
                "This is needed prior to performing transactions related to private keys such as sending graviocoin\n"
            "\nNote:\n"
...
static UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending graviocoin\n"
...
static UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    int nTxCount = (int)pwallet->mapWallet.size() + (pwallet->IsGraviocoinWallet() ? (int)((CHDWallet*)pwallet)->mapRecords.size() : 0);
    obj.pushKV("txcount",       (int)nTxCount);

    CKeyID seed_id;
    if (IsGraviocoinWallet(pwallet)) {
        CHDWallet *pwhd = GetGraviocoinWallet(pwallet);
...
static UniValue loadwallet(const JSONRPCRequest& request)
{
            RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting graviocoind will be"
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "A json array of graviocoin addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "graviocoin address"},
                        },
                    },
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the graviocoin address\n"
            "    \"coldstaking_address\"  : \"address\" (string) the graviocoin address this output must stake on\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Graviocoin address: ") + input.get_str());
            }
...
void FundTransaction(CWallet* const pwallet, CMutableTransaction& tx, CAmount& fee_out, int& change_position, UniValue options)
{
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid graviocoin address");
            }
    ...
    size_t nOutputs = IsGraviocoinWallet(pwallet) ? tx.vpout.size() : tx.vout.size();
    if (nOutputs == 0)
    ...
    RPCHelpMan{"fundrawtransaction",
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR, /* default */ "pool address", "The graviocoin address to receive the change"},
...
static UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    if (totalFee > 0 || IsGraviocoinWallet(pwallet)) {
        ...
        return NullUniValue;
            ...
            RPCHelpMan{"getaddressinfo",
                ...
                "Some of the information will only be present if the address is in the active wallet.\n",                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The graviocoin address to get the information of."},
                },
                RPCResult{
            "{\n"
            "  \"address\" : \"address\",        (string) The graviocoin address validated\n"
...
UniValue sethdseed(const JSONRPCRequest& request)
{
    ...
    if (IsGraviocoinWallet(pwallet))
        throw JSONRPCError(RPC_WALLET_ERROR, "Not necessary in Graviocoin mode.");
                ...
                "Implements the Creator and Updater roles.\n",
                {
                    ...
                    {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "a json array with outputs (key-value pairs), where none of the keys are duplicated.\n"
                            ...
                            "                             accepted as second parameter.",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                                    {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the graviocoin address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                                },
                                },
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                        {
                            {"changeAddress", RPCArg::Type::STR_HEX, /* default */ "pool address", "The graviocoin address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM, /* default */ "random", "The index of the change output"},
                            {"change_type", RPCArg::Type::STR, /* default */ "set by -changetype", "The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
                            {"includeWatching", RPCArg::Type::BOOL, /* default */ "true for watch-only wallets, otherwise false", "Also select inputs which are watch only"},
                            {"lockUnspents", RPCArg::Type::BOOL, /* default */ "false", "Lock selected unspent outputs"},
