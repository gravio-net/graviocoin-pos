...
static int ExtractBip32InfoV(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_SECRET_KEY)[0], 4) == 0) {
        keyInfo.pushKV("type", "Graviocoin extended secret key");
    } else
...
static int ExtractBip32InfoP(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_PUBLIC_KEY)[0], 4) == 0) {
        keyInfo.pushKV("type", "Graviocoin extended public key");
    } else
    ...
    if (mode == "import") {
        ...
        if (fBip44) {
            ...
            if (!eKey58.IsValid(CChainParams::EXT_SECRET_KEY)
                ...
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Import failed - Key must begin with a graviocoin prefix.");
            }
        }
...
static UniValue extkeyaltversion(const JSONRPCRequest &request)
{
            RPCHelpMan{"extkeyaltversion",
                ...
                "If the provided ext_key has a Bitcoin prefix the output will be encoded with a Graviocoin prefix.\n"
                "If the provided ext_key has a Graviocoin prefix the output will be encoded with a Bitcoin prefix.\n",
                {
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
            ...
            RPCHelpMan{"getnewextaddress",
                "\nReturns a new Graviocoin ext address for receiving payments." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                ...
                RPCResult{
            "\"address\"              (string) The new graviocoin extended address\n"
                },
                RPCExamples{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
            ...
            RPCHelpMan{"getnewstealthaddress",
                "\nReturns a new Graviocoin stealth address for receiving payments." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"label", RPCArg::Type::STR, /* default */ "", "If specified the key is added to the address book."},
            ...
            "           Stealth addresses with prefixes will scan only incoming stealth transactions with a matching prefix."},
                ...
                RPCResult{
            "\"address\"              (string) The new graviocoin stealth address\n"
                },
                RPCExamples{
...
static UniValue getstakinginfo(const JSONRPCRequest &request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
            ...
            "  \"moneysupply\": xxxxxxx,        (numeric) the total amount of graviocoin in the network\n"
        ...
        return NullUniValue;
            ...
            RPCHelpMan{"listunspentanon",
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "", "A json array of graviocoin addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, /* default */ "", "graviocoin address"},
                        },
                    },
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValidStealthAddress())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Graviocoin stealth address: ")+input.get_str());
    ...
    if (request.params[0].isArray()) {
        ...
        for (size_t k = 0; k < outputs.size(); ++k) {
            ...
            if (typeOut == OUTPUT_RINGCT
                ...
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Graviocoin stealth address");
            }
...
static std::string SendHelp(CHDWallet *pwallet, OutputTypes typeIn, OutputTypes typeOut)
{
    ...
    rv +=   "\nArguments:\n"
            "1. \"address\"     (string, required) The graviocoin address to send to.\n"
        ...
        return NullUniValue;
                    ...
                    {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of json objects",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::NO, "",
                                {
                                    {"address", RPCArg::Type::STR, /* default */ "", "The graviocoin address to send to."},
            ...
            "                             transaction, just kept in your wallet."},
                    ...
                    {"coin_control", RPCArg::Type::OBJ, /* default */ "", "",
                        {
                            {"changeaddress", RPCArg::Type::STR, /* default */ "", "The graviocoin address to receive the change"},
...
static UniValue createrawparttransaction(const JSONRPCRequest& request)
{
                    ...
                    {"outputs", RPCArg::Type::ARR, /* default */ "", "a json array with outputs (key-value pairs).",
                        {
                            {"", RPCArg::Type::OBJ, /* default */ "", "",
                                {
                                    {"address", RPCArg::Type::STR, /* default */ "", "The graviocoin address."},
    ...
    rawTx.nVersion = GIO_TXN_VERSION;
