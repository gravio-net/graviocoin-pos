...
const std::string GIO_EXTACC{"eacc"};
const std::string GIO_EXTKEY{"ek32"};
const std::string GIO_EXTKEYNAMED{"eknm"};
const std::string GIO_SXADDR{"sxad"};
const std::string GIO_SXADDRKEYPACK{"espk"};
const std::string GIO_LOCKEDUTXO{"luo"};
} // namespace DBKeys
             ...
             CWalletScanState &wss, std::string& strType, std::string& strErr) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet, pwallet->GetLegacyScriptPubKeyMan()->cs_wallet)
{
    try {
        ...
        if (strType == DBKeys::NAME) {
        ...
        } else if (strType == DBKeys::MASTER_KEY && !fGraviocoinMode) {
            ...
            return false;
        } else if (strType == DBKeys::GIO_LOCKEDUTXO) {
            COutPoint output;
            ssKey >> output;
...
bool WalletBatch::IsKeyType(const std::string& strType)
{
    return (strType == DBKeys::KEY ||
            ...
            || (fGraviocoinMode &&
                (strType == DBKeys::GIO_EXTACC || strType == DBKeys::GIO_EXTKEY
                || strType == DBKeys::GIO_EXTKEYNAMED || strType == DBKeys::GIO_SXADDR || strType == DBKeys::GIO_SXADDRKEYPACK));
}
