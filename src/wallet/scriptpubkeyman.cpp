...
isminetype IsMineInner(const LegacyScriptPubKeyMan& keystore, const CScript& scriptPubKey, bool& isInvalid, IsMineSigVersion sigversion)
{
    ...
    case TX_PUBKEY:
        ...
        if ((mine = keystore.m_graviocoin ? keystore.m_graviocoin->IsMine(keyID) : keystore.m_storage.IsMine(keyID)))
            return mine;
...
{
    if (m_graviocoin) return m_graviocoin->HaveKey(address);
    LOCK(cs_KeyStore);
    if (!m_storage.HasEncryptionKeys()) {
...
{
    if (m_graviocoin) return m_graviocoin->GetKey(address, keyOut);
    LOCK(cs_KeyStore);
    if (!m_storage.HasEncryptionKeys()) {
...
{
    if (m_graviocoin) return m_graviocoin->GetPubKey(address, vchPubKeyOut);
    LOCK(cs_KeyStore);
    if (!m_storage.HasEncryptionKeys()) {
...
bool LegacyScriptPubKeyMan::GetKeyFromPool(CPubKey& result, const OutputType type, bool internal)
{
    ...
    if (m_graviocoin) return m_graviocoin->GetKeyFromPool(result, internal);
