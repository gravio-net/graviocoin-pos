...
struct StakeTestingSetup: public TestingSetup {
    StakeTestingSetup(const std::string& chainName = CBaseChainParams::REGTEST):
        TestingSetup(chainName, /* fGraviocoinMode */ true)
    {
        ECC_Start_Stealth();
        ECC_Start_Blinding();
...
BOOST_AUTO_TEST_CASE(stake_test)
{
    ...
    BOOST_REQUIRE(::ChainActive().Tip()->nMoneySupply == 12501000000000);
    ...
    BOOST_REQUIRE(::ChainActive().Tip()->nMoneySupply == 12500500000000);
...

    {
...

        {
            ...
            BOOST_CHECK(::ChainActive().Tip()->nMoneySupply == 12501500034600);
        }
    }
    ...
    BOOST_CHECK(pwallet->GetBalance().m_mine_trusted + pwallet->GetStaked() == 12501499990000);
