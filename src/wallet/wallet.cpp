...
std::shared_ptr<CWallet> LoadWallet(interfaces::Chain& chain, const WalletLocation& location, std::string& error, std::vector<std::string>& warnings)
{
    ...
    if (fGraviocoinMode && !((CHDWallet*)wallet.get())->Initialise()) {
        error = "Graviocoin wallet initialise failed.";
        return nullptr;
    }
...
WalletCreationStatus CreateWallet(interfaces::Chain& chain, const SecureString& passphrase, uint64_t wallet_creation_flags, const std::string& name, std::string& error, std::vector<std::string>& warnings, std::shared_ptr<CWallet>& result)
{
    ...
    if (fGraviocoinMode && !GetGraviocoinWallet(wallet.get())->Initialise()) {
    ...
    if (!passphrase.empty() && !(wallet_creation_flags & WALLET_FLAG_DISABLE_PRIVATE_KEYS)) {
        ...
        if (!create_blank) {
            ...
            if (fGraviocoinMode) {
                if (0 != GetGraviocoinWallet(wallet.get())->MakeDefaultAccount()) {
...
CAmount CWalletTx::GetAvailableCredit(bool fUseCache, const isminefilter& filter) const
{
    ...
    for (unsigned int i = 0; i < tx->GetNumVOuts(); i++)
    {
        if (!pwallet->IsSpent(hashTx, i) && (allow_used_addresses || !pwallet->IsUsedDestination(hashTx, i))) {
            nCredit += pwallet->IsGraviocoinWallet()
...
std::shared_ptr<CWallet> CWallet::CreateWalletFromFile(interfaces::Chain& chain, const WalletLocation& location, std::string& error, std::vector<std::string>& warnings, uint64_t wallet_creation_flags)
{
    ...
    std::shared_ptr<CWallet> walletInstance(fGraviocoinMode
    ...
    if (gArgs.GetBoolArg("-upgradewallet", false) && !fGraviocoinMode) {
    ...
    if (!fGraviocoinMode) // Must rescan after hdwallet is loaded
...
int CWalletTx::GetBlocksToMaturity() const
{
    ...
    if (fGraviocoinMode && pwallet->m_last_block_processed_height < COINBASE_MATURITY * 2 && m_confirm.status == CWalletTx::Status::CONFIRMED) {
...
std::vector<OutputGroup> CWallet::GroupOutputs(const std::vector<COutput>& outputs, bool single_coin) const {
    ...
    for (const auto& output : outputs) {
        if (output.fSpendable) {
            ...
            const CScript *pscript = output.tx->tx->IsGraviocoinVersion()
