...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    state.SetStateInfo(nAcceptTime, ::ChainActive().Height(), consensus, fGraviocoinMode, (fBusyImporting && fSkipRangeproof));
    ...
    if (::GetSerializeSize(tx, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) < (fGraviocoinMode ? MIN_STANDARD_TX_NONWITNESS_SIZE_GIO : MIN_STANDARD_TX_NONWITNESS_SIZE))
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    const Consensus::Params &consensus = Params().GetConsensus();
    state.SetStateInfo(block.nTime, pindex->nHeight, consensus, fGraviocoinMode, (fBusyImporting && fSkipRangeproof));
    ...
    if (!fGraviocoinMode  // genesis coinbase is spendable when in Graviocoin mode
        && fIsGenesisBlock) {
        if (!fJustCheck)
    ...
    bool fEnforceBIP30 = fGraviocoinMode || (!((pindex->nHeight==91842 && pindex->GetBlockHash() == uint256S("0x00000000000a4d0a398161ffc163c503763b1f4360639393e0e4c8e300e0caec")) ||
    ...
    if ((fGraviocoinMode && pindex->pprev) || pindex->nHeight >= chainparams.GetConsensus().CSVHeight) {
    ...
    blockundo.vtxundo.reserve(block.vtx.size() - (fGraviocoinMode ? 0 : 1));
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        tx_state.SetStateInfo(block.nTime, pindex->nHeight, consensus, fGraviocoinMode, (fBusyImporting && fSkipRangeproof));
        if (!tx.IsCoinBase())
        {
            CAmount txfee = 0;
    ...
    if ((!fIsGenesisBlock || fGraviocoinMode)
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (!::ChainstateActive().IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            if (fGraviocoinMode)
            {
                if (pindex->nVersion > GIO_BLOCK_VERSION)
                    ++nUpgraded;
            } else
            {
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    state.SetStateInfo(block.nTime, -1, consensusParams, fGraviocoinMode, (fBusyImporting && fSkipRangeproof));
    ...
    for (const auto& tx : block.vtx) {
        TxValidationState tx_state;
        tx_state.SetStateInfo(block.nTime, -1, consensusParams, fGraviocoinMode, (fBusyImporting && fSkipRangeproof));
...
{
    if (fGraviocoinMode) return true;
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, bool accept_block=false) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    int nLockTimeFlags = 0;
    if ((fGraviocoinMode && pindexPrev) || nHeight >= consensusParams.CSVHeight) {
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        if (miSelf != m_block_index.end()) {
            ...
            if (fGraviocoinMode && !fRequested && !::ChainstateActive().IsInitialBlockDownload() && state.nodeId >= 0
    ...
    if (pindex == nullptr) {
        ...
        if (fGraviocoinMode && !::ChainstateActive().IsInitialBlockDownload() && state.nodeId >= 0) {
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (!pindex->pprev
            ...
            if (fGraviocoinMode && !IsInitialBlockDownload()) {
...
bool LoadMempool(CTxMemPool& pool)
{
    ...
    try {
        ...
        while (num--) {
            ...
            assert(tip);
            const Consensus::Params &consensus = Params().GetConsensus();
            state.SetStateInfo(tip->nTime, tip->nHeight, consensus, fGraviocoinMode, (fBusyImporting && fSkipRangeproof));
