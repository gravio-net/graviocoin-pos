...
1. Clone the Graviocoin Core source code:
    ```shell
    git clone https://github.com/graviocoin/graviocoin-core
    cd graviocoin-core
    ```

2.  Build Graviocoin Core:

    Configure and build the headless Graviocoin Core binaries as well as the GUI (if Qt is found).
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, Graviocoin Core may be
...
```shell
mkdir -p "/Users/${USER}/Library/Application Support/Graviocoin"

touch "/Users/${USER}/Library/Application Support/Graviocoin/graviocoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Graviocoin/graviocoin.conf"
```

The first time you run graviocoind, it will start downloading the blockchain. This process could
