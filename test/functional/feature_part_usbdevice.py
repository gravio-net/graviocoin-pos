...
from test_framework.test_graviocoin import (
    GraviocoinTestFramework,
    isclose,
    getIndexAtProperty,
    connect_nodes_bi,
)
...
class USBDeviceTest(GraviocoinTestFramework):
    ...
    def run_test(self):
        ...
        # Check that graviocoin has been built with USB device enabled
        ...
        if not config["components"].getboolean("ENABLE_USBDEVICE"):
            raise SkipTest("graviocoind has not been built with usb device enabled.")
