...
ALL_SCRIPTS = EXTENDED_SCRIPTS + BASE_SCRIPTS + GIO_SCRIPTS + INSIGHT_SCRIPTS + GIO_SCRIPTS_EXT
...
def main():
    ...
    parser.add_argument('--graviocoin', action='store_true', help='run Graviocoin specific tests')
    parser.add_argument('--graviocoinext', action='store_true', help='run Graviocoin extended tests')
    ...
    tmpdir = "%s/graviocoin_test_runner_P_🏃_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    ...
    #    test_list += ALL_SCRIPTS
        ...
        if args.graviocoin:
            test_list += GIO_SCRIPTS
        if args.insight:
            test_list += INSIGHT_SCRIPTS
        if args.bitcoin:
            test_list += BASE_SCRIPTS
        if args.graviocoinext:
            test_list += GIO_SCRIPTS_EXT
    ...
    run_tests(
        ...
        create_cache=(True if args.bitcoin or (not args.graviocoin and not args.insight) else False)
    )
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, runs_ci, use_term_control, create_cache=True):
    ...
    try:
        if subprocess.check_output(["pidof", "graviocoind"]) not in [b'']:
            print("%sWARNING!%s There is already a graviocoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
